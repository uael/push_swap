...
static inline void			onprint(char const *row, char const *pos)
{
	char *vis;
	...
	if (g_mode == RL_VISUAL)
	{
		...
		{
			if (pos > vis)
				ft_fputs(g_stdin, TC_ME);
			else if (pos < vis)
				ft_fputs(g_stdin, TC_MR);
		}
		else if (row == vis)
		{
			if (vis > pos)
	...
	char const *eol)
{
	while (r < eol)
	{
		...
		ft_fputc(g_stdin, (unsigned char)*r++);
	}
	onprint(r, p);
}
...
static inline uint16_t		printprompt(void)
{
	...
	while (*p)
	{
		...
		ft_fputc(g_stdin, (unsigned char)*p++);
	}
	return (i);
}
...
inline void					rl_editprint(void)
{
	...
	if (g_eln->row < g_eln->rows.len)
		ft_fprintf(g_stdin, TC_GOTODO(g_eln->rows.len - g_eln->row));
	i = 0;
	while (++i < g_eln->rows.len)
		ft_fputs(g_stdin, TC_CL_UP);
	ft_fputs(g_stdin, TC_CL);
	g_screen->col = 0;
	g_idx_up = 0;
	...
	if (g_idx_up)
		ft_fprintf(g_stdin, TC_GOTOUP(g_idx_up));
	g_eln->row = (uint16_t)(g_eln->rows.len - g_idx_up);
	if (!g_idx_col)
		ft_fputs(g_stdin, TC_CH);
	else
		ft_fprintf(g_stdin, TC_GOTOCH(g_idx_col));
